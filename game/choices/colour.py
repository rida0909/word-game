...
COLOUR_WORDS = ['DRE', 'OELLYW', 'LBEU', 'ERGNE', 'OREAGN', 'RPEULP', 'EMIL', 'ORWBN', 'NYAV', 'PNIK', 'LDGO', 'VRLSEI',
               'BLKAC', 'HTWIE', ]
...
word_num = randrange(0, (len(COLOUR_WORDS)))
jumbled_rand_word = COLOUR_WORDS[word_num]
...
def main():
    ...
    def change():
        global word_num
        word_num = randrange(0,(len(COLOUR_WORDS)))
        word.configure(text = COLOUR_WORDS[word_num])
    ...
    def check():
        ...
        if answer == COLOUR_ANSWER[word_num]:
            points +=5
    ...
    def show_answer():
        ...
        if points > 4:
            ...
            score.configure(text="score = " + str(points))
            time.sleep(0.5)
            ans_lab.configure(text = COLOUR_ANSWER[word_num])
        else:
