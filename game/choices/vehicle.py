...
VEHICLES_WORDS = ['RIHLPOEETC', 'NELAARIP', 'CKTREO', 'LITSAOBA', 'UIECRS PIHS', 'ROAGC SPHI', 'TJE SKI', 'PIREAT IHSP',
...
word_num = randrange(0, (len(VEHICLES_WORDS)))
jumbled_rand_word = VEHICLES_WORDS[word_num]
...
def main():
    ...
    def change():
        global word_num
        word_num = randrange(0,(len(VEHICLES_WORDS)))
        word.configure(text = VEHICLES_WORDS[word_num])
    ...
    def check():
        ...
        if answer == VEHICLES_ANSWER[word_num]:
            points +=5
    ...
    def show_answer():
        ...
        if points > 4:
            ...
            score.configure(text="score = " + str(points))
            time.sleep(0.5)
            ans_lab.configure(text = VEHICLES_ANSWER[word_num])
        else:
