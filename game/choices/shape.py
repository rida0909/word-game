...
SHAPES_WORDS = ['CCLIER', 'IDANDOM', 'RHATE', 'AOTNOCG', 'USQRAE', 'TARS', 'RITLANEG', ]

SHAPES_ANSWER = ['CIRCLE', 'DIAMOND', 'HEART', 'OCTAGON', 'SQUARE', 'STAR', 'TRIANGLE', ]

word_num = randrange(0, (len(SHAPES_WORDS)))
jumbled_rand_word = SHAPES_WORDS[word_num]
...
def main():
    ...
    def change():
        global word_num
        word_num = randrange(0,(len(SHAPES_WORDS)))
        word.configure(text = SHAPES_WORDS[word_num])
    ...
    def check():
        ...
        if answer == SHAPES_ANSWER[word_num]:
            points +=5
    ...
    def show_answer():
        ...
        if points > 4:
            ...
            score.configure(text="score = " + str(points))
            time.sleep(0.5)
            ans_lab.configure(text = SHAPES_ANSWER[word_num])
        else:
